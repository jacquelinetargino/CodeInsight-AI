...
usuário virando segmento de caminho de uma URL da GitHub API**. O `httpx` normaliza `..`
no momento em que constrói a `URL`, então um segmento a mais troca o endpoint chamado
sem que o código pareça errado — e a requisição sai com credencial.

O PR 35 corrigiu a primeira ocorrência validando **aquela** entrada. Foi o bastante para
o caso conhecido e insuficiente para a classe: havia outra entrada indo para o mesmo
lugar. Agora existe uma guarda geral no cliente, abaixo das validações específicas.

As rotas de autenticação — as únicas abertas ao público — também não tinham proteção
nenhuma contra abuso.

### Corrigido

- **`file_path` de `POST /analysis/{id}/fix` escapava do repositório.** O campo entra no
  caminho de uma URL da GitHub API e nada o validava. Medido, interceptando a requisição
  real, `../../../vitima/repo-privado/contents/.env` virava
...
- **O tempo de resposta do login dizia quem tem conta.** E-mail cadastrado respondia em
  213 ms e desconhecido em 0,9 ms — 236x de diferença, porque o `or` curto-circuitava
  antes do `verify_password`. A conferência agora roda nos dois casos, contra um hash
  descartável quando o e-mail não existe. Fecha só o canal de tempo: o 409 do cadastro
  continua identificando quem tem conta, e mudar isso exigiria confirmação por e-mail.
...
- **A análise falhada devolvia a exceção crua ao usuário.** `Analysis.error_message`
  ...
  um `FileNotFoundError` do motor entregava
  `C:\Users\<conta>\AppData\Local\Temp\codeinsight-a1b2c3\src\...` — caminho do diretório
  temporário e nome da conta que roda o servidor; um `KeyError` chegava como
  `'chave_que_nao_existe'`, que não informa nada. E como mensagem de erro de sistema de
  arquivos carrega o nome do arquivo, o repositório analisado (conteúdo não confiável)
  escolhia parte do texto exibido — não era XSS, porque o React escapa, mas era um
  caminho até a interface que ninguém tinha desenhado. Agora só a mensagem escrita para
  o usuário passa; o resto vira texto genérico e o detalhe fica no log. A recusa da
  GitHub API é traduzida por status em vez de escondida: 404 explica que o repositório
  pode ser privado e como conectar um token.
- **O download do tarball aceitava redirecionamento para `http://`.** A allowlist conferia
  ...
  num segmento trocaria o endpoint chamado. Nenhuma entrada chega lá com `..` hoje, e a
  guarda existe porque nas duas ocorrências anteriores o que faltava era exatamente ela.
- **O 429 chegava ao usuário como "Erro 429".** O `slowapi` responde com `{error: ...}` e
  não com o `detail` do FastAPI, então o frontend não achava mensagem nenhuma. Passava
  despercebido enquanto o limite só existia em rotas caras; com o limite no login, quem
  errasse a senha dez vezes veria só o número.
- **O teste que garante que o relatório não avalia template dava alarme falso.** Ele
  ...
  mais no relatório produz. A capacidade de detecção não mudou: reintroduzindo a
  avaliação de propósito, os três payloads continuam sendo pegos.
- **Três das quatro declarações de versão estavam paradas na 0.1.0.** Só o selo do README
  era atualizado a cada release. O `FastAPI(version=...)` é o que mais incomodava: ele
  aparece em `/docs` e no `/openapi.json`, ou seja, a API informava a quem a consome uma
  versão que não era a dela.
...
- **`app.__version__` é a fonte única da versão**, e `tests/test_versao.py` confere que
  as quatro declarações concordam e que existe entrada no CHANGELOG para a versão
  declarada — subir a versão sem dizer o que mudou é a outra metade do problema.
- **Sentinela de teste de injeção precisa ser improvável**, verificado por teste: nenhuma
  pode aparecer num relatório sem payload, e todas têm no mínimo 8 caracteres. As duas
  que quebraram tinham 2 e 6.
...
- **O `Authorization` é reenviado em todos os saltos do download do tarball**, ao
  contrário do que o httpx faz sozinho ao trocar de origem. Não foi alterado: os três
  hosts permitidos são do GitHub, e verificar se o `codeload` precisa do token para
  entregar tarball de repositório privado exige um repositório privado de teste. Mudar
  sem medir arriscaria quebrar a análise de repositório privado. Critério para resolver:
  medir primeiro.
- **O limitador continua contando na memória do processo.** Com mais de uma instância, o
  limite efetivo é multiplicado pelas réplicas — inclusive o do login. Herdado da 0.3.3.
